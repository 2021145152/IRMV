...
final_objects_template.json을 사용하여 객체의 모든 속성을 가져옵니다.
unique_scene_categories.json의 모든 방 카테고리를 지원하며,
final_objects_with_affordance.json의 모든 95개 객체를 적절한 방에 분류합니다.
...
# final_objects_with_affordance.json의 모든 95개 객체를 적절한 방에 분류했습니다.
ROOM_OBJECT_MAPPING = {
    "bedroom": [
...
def load_object_templates(templates_path: Optional[Path] = None) -> Dict[str, Dict]:
    """
    final_objects_template.json 파일을 로드
    ...
    if templates_path is None:
        # 스크립트 디렉토리에서 final_objects_template.json 찾기
        script_dir = Path(__file__).parent
        templates_path = script_dir / "final_objects_template.json"
        
        # 없으면 상위 디렉토리에서 찾기
        if not templates_path.exists():
            templates_path = script_dir.parent / "final_objects_template.json"
...
def generate_objects_for_room(room_id: int, room_data: Dict, existing_objects: Dict,
    ...
    # safe-key 연결은 모든 방 생성 후 전역적으로 처리 (다른 방에 배치하기 위해)
    # 여기서는 생성된 객체만 반환
...
def connect_safes_and_keys_globally(existing_objects: Dict, rooms: Dict) -> int:
    """
    모든 방의 객체 생성 후 전역적으로 safe-key 연결 처리
    전체 방에서 랜덤으로 선택하여 생성
    
    Args:
        existing_objects: 모든 객체 딕셔너리
        rooms: 모든 방 딕셔너리
    ...
    # safe가 생성되었는데 대응하는 key가 없으면 다른 방에 key 생성
    for safe_id_str, safe_data in safes:
        # 이미 연결된 key가 있는지 확인
        connected_key = None
        for key_id_str, key_data in keys:
            if key_data.get("unlocks") == int(safe_id_str):
                connected_key = (key_id_str, key_data)
                break
        
        if connected_key:
            # 이미 연결된 key가 있으면 requires_key 설정만 업데이트
            key_id_str, key_data = connected_key
            safe_data["requires_key"] = int(key_id_str)
            safe_data["is_locked"] = True
            print(f"  🔗 Safe (ID: {safe_id_str}) already connected to key (ID: {key_id_str})")
        else:
            # 전체 방에서 랜덤으로 선택하여 key 생성
            all_rooms = list(rooms.items())
            if not all_rooms:
                print(f"  ⚠️  No rooms available for key generation")
                continue
            
            # 랜덤하게 방 선택
            room_id_str, room_data = random.choice(all_rooms)
            ...
            # key 생성
            key_id = max_existing_id + new_objects_count + 1
            ...
            if key_data:
                ...
                existing_objects[str(key_id)] = key_data
                keys.append((str(key_id), key_data))
                new_objects_count += 1
                
                # safe의 requires_key와 is_locked 설정
                safe_data["requires_key"] = key_id
                safe_data["is_locked"] = True
                safe_room_id = safe_data.get("parent_room")
                print(f"  🔑 Generated key (ID: {key_id}) in room {room_id} for safe (ID: {safe_id_str}) in room {safe_room_id}")
    
    # key가 생성되었는데 대응하는 safe가 없으면 다른 방에 safe 생성
    for key_id_str, key_data in keys:
        # 이미 연결된 safe가 있는지 확인
        if key_data.get("unlocks") is None:
            # 전체 방에서 랜덤으로 선택하여 safe 생성
            all_rooms = list(rooms.items())
            if not all_rooms:
                print(f"  ⚠️  No rooms available for safe generation")
                continue
            ...
            # safe 생성
            safe_id = max_existing_id + new_objects_count + 1
            ...
            if safe_data:
                ...
                safe_data["requires_key"] = int(key_id_str)
                safe_data["is_locked"] = True
                existing_objects[str(safe_id)] = safe_data
                safes.append((str(safe_id), safe_data))
                new_objects_count += 1
                key_room_id = key_data.get("parent_room")
                print(f"  🔒 Generated safe (ID: {safe_id}) in room {room_id} for key (ID: {key_id_str}) in room {key_room_id}")
...
def generate_objects_in_rooms(json_path: Path, 
    ...
    # 모든 방 생성 후 전역적으로 safe-key 연결 처리
    print(f"\n🔗 Connecting safes and keys globally...")
    new_objects_count = connect_safes_and_keys_globally(existing_objects, rooms)
    total_generated += new_objects_count
...
def main():
    ...
    parser.add_argument(
        "--templates", "-t",
        type=str,
        help="final_objects_template.json 파일 경로 (지정하지 않으면 자동으로 찾기)"
    )
    ...
    if args.json:
        ...
        json_files = [
            ...
            and f.name != "final_objects_template.json"  # 템플릿 파일 제외
        ]
