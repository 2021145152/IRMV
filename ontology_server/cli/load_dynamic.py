...
class DynamicLoader:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if failed_count > 0:
                        print(f"WARNING: Failed to load {failed_count} individuals")
                    print(f"Status: Status: {result.get('status', 'unknown')}")
                else:
                    error_detail = response.json().get("detail", "Unknown error")
                    print(f"ERROR: Request failed: {error_detail}")
    ...
    def check_server(self) -> bool:
        ...
        try:
            ...
            print(f"ERROR: Cannot connect to server at {self.api_url}")
...
def main():
    ...
    if not active_env:
        print("ERROR: No active space configured in config.yaml")
    ...
    if not ttl_path:
        print(f"ERROR: Environment '{active_env}' not found or has no dynamic.ttl file")
        sys.exit(1)
